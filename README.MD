...
### **Privates methods**
```c++
    bool success = _MFOpenBackdoor(std::string &status, bool withUnselect = true);
```

### **Publics methods**
```c++
    bool success = MFSevenByteUidFirstInit(std::vector<int> key, std::string &status, int typeFn = 1);
    bool success = MFChangeUid(std::vector<int> key, std::vector<int> datas, std::string &status);;
    bool success = MFUnbrickUid(std::string &status);
```
## **INITIALIZE**
```c++
#include <mfrc522.h>
